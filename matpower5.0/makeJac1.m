function [J, Ybus, Yf, Yt] = makeJac1(baseMVA, bus, branch, gen, fullJac)
...
if fullJac
    ...
    pv = [ref;pv];
    act = [pq;pv];
%     want = [35 5	28	10	17	21	37]';% 1]';
%        want = [35 2:1:8, 26:30 10 17 21 37];
% want = [2:1:3 5:8 26:1:30 35 10 17 21 37];
%     ind = [];
%     for k = 1:numel(want)
%         ind = [ind;find(act==want(k))];
%     end
    
    
    j11 = real(dSbus_dVa([pq; pv], [pq; pv]));
    j12 = real(dSbus_dVm([pq; pv], [pq; pv]));
    j21 = imag(dSbus_dVa([pq; pv], [pq; pv]));
    j22 = imag(dSbus_dVm([pq; pv], [pq; pv]));
else
